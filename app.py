...
review (full note editable, keywords as reference sidebar) -> patient record save.
...
def process_patient_intake(patient_id, language, audio_file):
    ...
    4. Return: full note (editable) + keywords (reference sidebar) for doctor review
    """
...
"""
    ...
    # Prepare patient record with editable note + reference keywords
    patient_record = {
    ...
    return full_note_text, keywords_text, json.dumps(patient_record, indent=2), "✓ Ready for review"
...
with gr.Blocks(title="NCAIR-DSA: Patient Intake Assistant") as demo:
    ...
    gr.Markdown("**Left: Full clinical note (editable).** | **Right: Quick reference keywords (read-only sidebar).**")
    ...
    with gr.Row():
        ...
        # Sidebar: keywords reference (non-editable)
    ...
    transcribe_btn.click(
        ...
        outputs=[full_note, keywords_display, patient_record_json, save_status]
    )
